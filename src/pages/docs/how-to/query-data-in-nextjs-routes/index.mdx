...
Create a new file in your Next.js `pages` directory. For this example, we'll make a two pages called `ssr.js` & `ssg.js`. We'll add a simple component to confirm the page can be navigated to.
...
Server-side rendering (SSR) is useful when you need to fetch fresh data on every request. This is beneficial for pages that are user specific dashboard or a page that displays real-time data.
