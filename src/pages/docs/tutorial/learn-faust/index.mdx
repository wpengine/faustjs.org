...
This tutorial will familiarize you with the core features of Faust.js. You'll start with a Next.js project that has been configured to work with Faust.js, then learn how to implement these features yourself:

- Template hierarchy
- Authentication & Post previews
- Blocks

After going through this tutorial, you'll be able to [configure your Next.js app]('/docs/how-to-basic-setup'), then use the knowledge you gained to implement Faust.js' features in your own apps.
...
// TODO: Update this repo link

```sh
git clone git@github.com:kellenmace/faust-tutorial.git
```
...
Run `npx @wp-now/wp-now start --blueprint faust-tutorial-blueprint.json`. This command uses the [`wp-now` NPM package](https://www.npmjs.com/package/@wp-now/wp-now) and the `faust-tutorial-blueprint.json` blueprint file to set up a local WordPress environment.
...
By navigating to http://localhost:8881/wp-admin/plugins.php, You can see that the required [Faust.js](https://wordpress.org/plugins/faustwp/) and [WPGraphQL](https://wordpress.org/plugins/wp-graphql/) plugins have already been installed and activated for you.

In the WordPress admin sidebar, go to `Settings` > `Permalinks`. Check the radio button for `Post name` and save your changes.

Next, head to `GraphQL` > `Settings`. Check the box for `Enable Public Introspection` and click the save button.
...
Run `npm run dev` to install the project's NPM packages
...
FAUST_SECRET_KEY=abc-123
```

In a separate terminal window from the one running your WordPress backend, run `npm run dev` to start up your frontend app. You should now be able to visit http://localhost:3000 and see your frontend app running.
...
Open the project in a code editor and find the `wp-templates` folder. This is the folder where you can place page template files you'd like to use. We've provided a `single.js` file for you. As you can see in the template hierarchy flowchart, the "single" page template is used to render single blog post pages. You can see this page template in action by viewing http://localhost:3000/hello-world/. That page should display the contents for the "Hello world" blog post.
...
The last step is to make Faust aware of our new template. Do to this, open `wp-templates/index.js` and add these lines:
...
### 1. Create the Faust API route

First, we'll set up an API endpoint that Faust uses to authenticate requests from WordPress.

Create a new `api` folder inside of your `pages` folder. Inside of `/api`, add a `faust/[[...route]].js` file that contains the code below.
...
Next, we'll create a new Next.js page to handle post previews. Create a new `pages/preview.js` file with this inside:

```js title="src/pages/preview.js"
...
With this code in place, Faust will render the correct `WordPressTemplate` page template according to the template hierarchy and pass to it an additional `asPreview` prop that is set to `true`. Next, we'll see how to use that prop to determine whether a post preview is being rendered in our page templates.

### 3. Update `single` template
...
After you complete the tutorial, you can review our how-to guides on [Authentication](/docs/how-to/authentication/) and [Post Previews](/docs/how-to/post-previews/) to learn more about these features.
...
In the root of your Next.js project, create a new `src/wp-blocks/index.js` file with this:

```js title="src/wp-blocks/index.js"
...
This get the WordPress core blocks from the `@faustwp/blocks` package and tells Faust to use them for rendering blocks. It's possible to override these blocks, or add your own custom blocks, but we'll just stick with the core blocks for now.

Let's update our `single` blog post template to query for and render blocks. Currently, the `single` template does this:
...
You should now be able to visit the URL for a post such as http://localhost:3000/blog/hello-world/ and see the blocks content being rendered.
...
More information about Faust's blocks features can be found in these how-to guides:
